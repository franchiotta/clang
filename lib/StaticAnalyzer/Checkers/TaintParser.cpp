...
namespace taintutil {
  ...
  TaintParser::process(){
    ...
    if(!executeXpathExpression(doc, BAD_CAST
          "/TaintChecker/TaintSources/TaintSource", &TaintParser::parseSources))
      return Errors::GeneralError;
    ...
    if(!executeXpathExpression(doc, BAD_CAST
         "/TaintChecker/TaintFilters/TaintFilter", &TaintParser::parseFilters))
      return Errors::GeneralError;
	...
	void 
	TaintParser::parseSources(xmlNodeSetPtr nodes){
    xmlNodePtr cur;
    int size;
  ...
  void
    ...
    for(int i = 0; i < size; ++i) {
      ...
      if(nodes->nodeTab[i]->type == XML_ELEMENT_NODE) {
        ...
        while (node != cur -> last) {
          if (xmlStrEqual(node -> name, xmlCharStrdup("method"))){
            propagateMethod = std::string(reinterpret_cast<char*>(node->children
                                                                   -> content));
          }
          if (xmlStrEqual(node -> name, xmlCharStrdup("sources"))){
            ...
            while(paramsNodes != node -> last){
              if (xmlStrEqual(paramsNodes -> name, xmlCharStrdup("value"))){
                pr.addSrcArg(std::stoi(reinterpret_cast<char*>(paramsNodes ->
                                                         children -> content)));
              }
              paramsNodes = paramsNodes -> next;
            }
          ...
          if (xmlStrEqual(node -> name, xmlCharStrdup("destinations"))){
            ...
            while(paramsNodes != node -> last){
              if (xmlStrEqual(paramsNodes -> name, xmlCharStrdup("value"))){
                pr.addDstArg(std::stoi(reinterpret_cast<char*>(paramsNodes ->
                                                          children -> content)));
              }
              paramsNodes = paramsNodes -> next;
            }
        ...
        propagationRuleMap.push_back(std::pair<std::string,PropagationRule>(
                                                          propagateMethod, pr));
      } else {
        cur = nodes->nodeTab[i];
      }
	...
	void
	TaintParser::parseDestinations(xmlNodeSetPtr nodes){
    xmlNodePtr cur;
    int size;
    ...
    for(int i = 0; i < size; ++i) {
      ...
      if(nodes->nodeTab[i]->type == XML_ELEMENT_NODE) {
        ...
        while (node != cur -> last) {
          if (xmlStrEqual(node -> name, xmlCharStrdup("method"))){
            destinationMethod = std::string(reinterpret_cast<char*>(node ->
                                                          children -> content));
          }
          if (xmlStrEqual(node -> name, xmlCharStrdup("params"))){
            ...
            while(paramsNodes != node -> last){
              if (xmlStrEqual(paramsNodes -> name, xmlCharStrdup("value"))){
                destinationArgs.push_back(std::stoi(reinterpret_cast<char*>(
                                          paramsNodes -> children -> content)));
              }
              paramsNodes = paramsNodes -> next;
            }
        ...
        destinationMap.push_back(std::pair<std::string,SmallVector<int,
                                SIZE_ARGS>>(destinationMethod,destinationArgs));
      } else {
        cur = nodes->nodeTab[i];
      }
    }
	...
	void
	TaintParser::parseFilters(xmlNodeSetPtr nodes){
    xmlNodePtr cur;
    int size;
    ...
    for(int i = 0; i < size; ++i) {
      ...
      if(nodes->nodeTab[i]->type == XML_ELEMENT_NODE) {
        ...
        while (node != cur -> last) {
          if (xmlStrEqual(node -> name, xmlCharStrdup("method"))){
            filterMethod = std::string(reinterpret_cast<char*>(node ->
                                                          children -> content));
          }
          if (xmlStrEqual(node -> name, xmlCharStrdup("params"))){
            xmlNodePtr paramsNodes = node -> children;
            while(paramsNodes != node -> last){
              if (xmlStrEqual(paramsNodes -> name, xmlCharStrdup("value"))){
                filterArgs.push_back(std::stoi(reinterpret_cast<char*>(
                                          paramsNodes -> children -> content)));
              }
              paramsNodes = paramsNodes -> next;
            }
	...
	bool
	TaintParser::validateXMLAgaintSchema(xmlDocPtr doc){
